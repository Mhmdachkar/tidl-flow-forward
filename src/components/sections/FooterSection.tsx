  ...
  return (
    ...
    >
      <div ref={moveRef} className="will-change-transform">
        <div
          ref={wordmarkRef}
          className="wordmark max-w-[100vw] px-4 font-display text-center font-black select-none will-change-transform"
      ...
      <div className="text-white px-5 pt-10 pb-8 sm:px-6 sm:pt-14 sm:pb-10" style={{ background: "#111111", borderRadius: "2rem 2rem 0 0" }}>
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-5">
          <div className="footer-card col-span-2 sm:col-span-1 flex flex-col gap-6 p-5 sm:gap-8 sm:p-6" style={{ background: "rgba(243,195,0,0.07)", borderRadius: "1.25rem", border: "1px solid rgba(243,195,0,0.25)" }}>
            <div>
              <p className="tidl-eyebrow mb-3" style={{ color: "#F3C300" }}>Get started today</p>
              <p className="tidl-display text-2xl leading-tight">Total care.<br />Totally different.</p>
          ...
          <div className="p-5 sm:p-6">
            <p className="tidl-eyebrow mb-4" style={{ color: "#F3C300" }}>Learn</p>
            <ul className="space-y-2.5 mb-8">
              {learnLinks.map((l) => (<li key={l}><a href="#" className="footer-link text-sm text-white/80">{l}</a></li>))}
            </ul>
            <p className="tidl-eyebrow mb-4" style={{ color: "#F3C300" }}>Tools</p>
          ...
          <div className="p-5 sm:p-6">
            <p className="tidl-eyebrow mb-4" style={{ color: "#F3C300" }}>Popular</p>
            <ul className="space-y-2.5">
              {popularLinks.map((l) => (
                <li key={l.label}>
          ...
          <div className="p-5 sm:p-6">
            <p className="tidl-eyebrow mb-4" style={{ color: "#F3C300" }}>TIDL</p>
            <ul className="space-y-2.5 mb-8">
              {tidlLinks.map((l) => (
                <li key={l.label}>
                  <a href="#" className="footer-link text-sm text-white/80 leading-tight">{l.label}</a>
            ...
            <p className="tidl-eyebrow mb-4" style={{ color: "#F3C300" }}>Careers</p>
            <ul className="space-y-2.5 mb-8">
              {careersLinks.map((l) => (<li key={l.label}><a href="#" className="footer-link text-sm text-white/80">{l.label}</a></li>))}
            </ul>
            <p className="tidl-eyebrow mb-4" style={{ color: "#F3C300" }}>Connect</p>
            <ul className="space-y-2.5">
